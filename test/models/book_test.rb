...
class BookTest < ActiveSupport::TestCase
  ...
  context "With a proper context, " do
    ...
    should "not allow for a published date in the past after the contract date" do
      # take advantage of the fact that the default contract date is 10 months ago...
      big_ruby_book = FactoryGirl.build(:book, published_date: 11.months.ago, category: @ruby, title: "The Big Book of Ruby")
      deny big_ruby_book.valid?
    end
    
    should "allow for just the published date to be nil" do
      big_ruby_book = FactoryGirl.build(:book, proposal_date: nil, contract_date: 30.weeks.ago, published_date: nil, category: @ruby, title: "The Big Book of Ruby")
      deny big_ruby_book.valid?
    end
    
    should "not allow for a published date in the past before the contract date" do
      big_ruby_book = FactoryGirl.build(:book, contract_date: 30.weeks.ago, published_date: 40.weeks.ago, category: @ruby, title: "The Big Book of Ruby")
      deny big_ruby_book.valid?
    end
    
    should "not allow for a published date in the future" do
      big_ruby_book = FactoryGirl.build(:book, published_date: 1.week.from_now, category: @ruby, title: "The Big Book of Ruby")
      deny big_ruby_book.valid?
    end
